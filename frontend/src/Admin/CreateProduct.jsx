...
import { addProduct } from "./AuthServieAdmin";
...
try {  
const { success, message } = await addProduct(productData);
    ...
    if (success) {
      ...
      setMessage("Product added successfully!"); // Affiche un message de succès.
      navigate("/dashboard/admin/product");  // Redirige l'utilisateur vers une autre page.
    } else {
      setMessage("Failed to add product"); // Affiche un message d'échec en cas de problème
    }
  }  catch (error) {
                ...
                >
                  ...
                  <div className="sm:col-span-4">
                       ...
                       <input
                           ...
                           value={formData.quantity}
                           onChange={handleChange}
                           min="1" // Minimum de 1 pour être positif
                           step="1" // Pour empêcher l'utilisateur d'ajouter des fractions
                           onInput={(e) => {
